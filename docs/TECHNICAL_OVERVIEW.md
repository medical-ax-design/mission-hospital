...
7. 공식 지도 이미지는 삼성서울병원 HTTPS URL에서 읽고 공개
   자료 상태에서는 경로 SVG를 만들지 않는다.
...
현재 프로토타입은 본관 지하 3층~20층, 별관 지하 3층~8층, 암병원
지하 3층~11층, 양성자치료센터 지하 3층·지하 1층의 공식 안내
이미지와 원문을 제공한다. 공개 이미지에는 이동 가능한 복도 그래프가
없으므로 모두 `MAP_ONLY`로 처리하고 경로선을 표시하지 않는다.
