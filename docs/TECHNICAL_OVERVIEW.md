...
7. 공식 지도 이미지는 삼성서울병원 HTTPS URL에서 읽고, 붉은 경로는
   원본 지도와 같은 비율의 SVG 계층으로 그린다. 붉은 사용자 표식은
   해당 선을 따라 이동한다.
8. 건물·층 이동은 공개 자료에서 확인된 연결 그래프만 탐색하며
   경로가 없으면 안내 데스크 문의 상태를 반환한다.
...
현재 프로토타입은 본관·별관·암병원의 공식 1~3층 안내 이미지 중
시나리오에 필요한 지도를 사용한다. 사용자가 건물·층·랜드마크를
선택한 `LocationFix`와 확인된 `본관 1F↔별관 1F`, `별관
1F↔암병원 2F` 연결을 사용하며, 건물 연결과 층 이동을 별도 화면으로
분리한다. 공개 자료로 연결이 확인되지 않은 에스컬레이터 경로는
비활성화한다. 자동 실내 위치는 별도 병원
