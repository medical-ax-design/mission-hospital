...
8. 경로 SVG는 `HOSPITAL_VERIFIED` 출처의 2D 지도·복도 노드와
   층별 선분이 모두 검증된 경우에만 렌더링한다.
9. 발표 프로토타입의 암병원 1층 세 목적지는 별도 클라이언트 시연
   그래프를 사용한다. 지도 터치 좌표를 가까운 복도 노드에 맞추고
   가중 최단 경로를 계산하지만 `HOSPITAL_VERIFIED`로 승격하지
   않는다.
...
없으므로 기본 상태는 `MAP_ONLY`다. 다만 암병원 1층의 내시경실,
원무수납, 의무기록복사 목적지와 본관 2층 채혈실에서 외래
엘리베이터를 거쳐 본관 1층 원무수납/접수로 이동하는 구간은 발표용
복도 그래프와 수동 현 위치 선택을 제공한다. 화면에서 시연·현장 검증
필요 상태를 명시하며 병원 승인 경로로 취급하지 않는다. 공식 안내도의
