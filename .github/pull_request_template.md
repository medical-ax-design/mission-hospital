## 변경 목적

<!-- 해결하려는 문제와 사용자·운영자에게 생기는 변화를 적어주세요. -->

## 변경 내용

### 포함

-

### 포함하지 않음

-

## 관련 문서

<!-- PRD 요구사항, 설계 문서, 이슈를 연결하세요. 없으면 '해당 없음'으로 적으세요. -->

-

## 검증

### 자동 검증

- [ ] `npm test`
- [ ] `npm run typecheck`
- [ ] `npm run build`
- [ ] `npm audit --audit-level=high`

### 수동 검증

- [ ] 핵심 사용자 흐름 확인
- [ ] 오류·빈 상태 확인

실행 결과:

```text
명령과 결과를 적어주세요.
```

## 데이터베이스·API

- Migration: 해당 없음
- API 계약 변경: 해당 없음
- 하위 호환성: 해당 없음
- Rollback 또는 복구: 해당 없음

## 보안·개인정보·의료 안전

- [ ] 실제 환자정보를 포함하지 않았습니다.
- [ ] 비밀정보와 토큰을 커밋하지 않았습니다.
- [ ] 인증·권한·조직 격리 영향을 검토했습니다.
- [ ] 의료지침을 임의로 생성하거나 변경하지 않았습니다.
- [ ] `READY`를 임상적 판단으로 표현하지 않았습니다.

## 화면 변경

<!-- 스크린샷 또는 영상을 첨부하세요. UI 변경이 없으면 '해당 없음'으로 적으세요. -->

해당 없음

접근성·반응형 확인:

- [ ] 키보드 접근
- [ ] 모바일 또는 데스크톱 기준 화면
- [ ] 오류 메시지와 포커스
- [ ] 스크롤 중 고정 UI와 콘텐츠가 겹치지 않음
- [ ] `prefers-reduced-motion` 적용 여부

## 외부 자료·콘텐츠

- [ ] 외부 이미지·문서·데이터의 원문 URL과 확인일을 기록했습니다.
- [ ] 운영 주체가 변경할 수 있는 장소·절차·의료 콘텐츠를 코드에서
      추측하지 않았습니다.
- [ ] 외부 원격 자산의 로딩 실패와 변경 위험을 검토했습니다.
- [ ] 프로토타입 경로·가상 데이터·공식 정보의 경계를 화면과 문서에
      표시했습니다.

## 배포·운영

- 환경변수 변경: 해당 없음
- 배포 순서: 해당 없음
- Health check 영향: 해당 없음
- Rollback: 해당 없음

배포 확인:

- [ ] Preview 또는 Production URL에서 핵심 흐름을 재검증했습니다.
- [ ] Web Origin과 API CORS 설정을 확인했습니다.
- [ ] API health check와 브라우저 콘솔 오류를 확인했습니다.
- [ ] 비밀정보 없이 롤백 가능한 배포 단위를 확인했습니다.

## 리뷰 요청

<!-- 리뷰어가 우선 확인할 파일, 규칙, 위험을 적어주세요. -->

-

## 완료 체크

- [ ] 변경 범위와 제외 범위를 명시했습니다.
- [ ] 관련 문서를 갱신했습니다.
- [ ] 필요한 테스트를 추가하거나 갱신했습니다.
- [ ] Draft 해제 전에 미완료 항목을 처리했습니다.
